...
%     norm, i.e. ``norm(a, 'fro')``.
  ...
  methods
    ...
    %     generated by ``eval_fn()``.
      if (nargin == 0) 
        obj.final_eval = @(~) struct('f_s', @(x) 0, 'f_n', @(x) 0, 'grad_f_s', @(x) zeros(size(x)), 'prox_f_n', @(x, lam) x);
      elseif (nargin == 4) 
        obj.final_eval = @(~) struct('f_s', @(x) feval(varargin{1}, x), 'f_n', @(x) feval(varargin{2}, x), ...
                                     'grad_f_s', @(x) feval(varargin{3}, x), 'prox_f_n', @(x, lam) feval(varargin{4}, x, lam));
      elseif (nargin == 1) 
