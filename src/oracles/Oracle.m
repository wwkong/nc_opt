classdef Oracle < matlab.mixin.Copyable & dynamicprops
  ...
  properties (SetAccess = protected, Hidden = true)
    ...
    pure_eval
    % Function that processes the evaluation of pure_eval at x.
    post_process
  end
  ...
  methods
    ...
    function add_smooth_oracle(obj, other_obj)
      ...
      %
      pp_cpy = obj.post_process;
      function Fn = smooth_add_process(Fn, x)
        other_obj.eval(x);
        Fn = pp_cpy(Fn, x);
        Fn.f_s = @(x) Fn.f_s(x) + other_obj.f_s();
        Fn.grad_f_s = @(x) Fn.grad_f_s(x) + other_obj.grad_f_s();
      end
      obj.post_process = @smooth_add_process;
    end
    ...
    function scale(obj, alpha)
      ...
      %
      pp_cpy = obj.post_process;
      function Fn = scale_process(Fn, x)
        Fn = pp_cpy(Fn, x);
        Fn.f_s = @(x) alpha * Fn.f_s(x);
        Fn.f_n = @(x) alpha * Fn.f_n(x);
        Fn.grad_f_s = @(x) alpha * Fn.grad_f_s(x);
        Fn.prox_f_n = @(x, lam) Fn.prox_f_n(x, lam * alpha);
      end
      obj.post_process = @scale_process;
    end
    ...
    function add_prox(obj, x_hat)
      ...
      %
      pp_cpy = obj.post_process;
      function Fn = add_prox_process(Fn, x)
        Fn = pp_cpy(Fn, x);
        Fn.f_s = @(x) Fn.f_s(x) + (1/2) * obj.norm_fn(x - x_hat)^2;
        Fn.grad_f_s = @(x) Fn.grad_f_s(x) + (x - x_hat);
      end
      obj.post_process = @add_prox_process;
    end
    ...
    function [f_s, f_n, grad_f_s, prox_f_n] = decompose(obj)
      ...
      % ``decompose()``.
      %
      f_s = @(x) feval(subsref(obj.pure_eval(x), ...
        struct('type','.','subs','f_s')), x);
      f_n = @(x) feval(subsref(obj.pure_eval(x), ...
        struct('type','.','subs','f_n')), x);
      grad_f_s = @(x) feval(subsref(obj.pure_eval(x), ...
        struct('type','.','subs','grad_f_s')), x);
      % Multi-input functions
      prox_f_n = @(x, lam) feval(subsref(obj.pure_eval(x), ...
