classdef Oracle < matlab.mixin.Copyable & dynamicprops
  ...
  methods
    ...
    function obj = Oracle(varargin)
      ...
      if (nargin == 0) % default constructor with dummy values
        obj.pure_eval = @(~) struct(...
        'f_s', @(x) 0, ...
        'f_n', @(x) 0, ...
        'grad_f_s', @(x) zeros(size(x)), ...
        'prox_f_n', @(x, lam) prox_f_n(x, lam));
      ...
      elseif (nargin == 1) % evaluator type constructor
        obj.pure_eval = varargin{1};
      else
        error(['Incorrect number of arguments: ', num2str(nargin)]);
      end
      obj.final_eval = @(x) obj.pure_eval(x);
    end
  ...
  properties (SetAccess = protected, Hidden = true)
    ...
    pure_eval
    % A generator that uses ``pure_eval`` and additional transforms to 
    % obtain the final oracle.
    final_eval
  end
  ...
  methods
    ...
    function obj = eval(obj, x)
      ...
      % The general form is ``a(F(x)) + b(x)``, where F(x) is property that
      % is generated by ``obj.eval_proxy``.
      obj.eval_point = x;
      Fn = obj.final_eval(x);
      obj.f_s = @() Fn.f_s(x);
      obj.f_n = @() Fn.f_n(x);
      obj.grad_f_s = @() Fn.grad_f_s(x);
      obj.prox_f_n = @(lam) Fn.prox_f_n(x, lam);
    end
    ...
    function add_smooth_oracle(obj, other_obj)
      ...
      function smooth_add_Fn = smooth_add_eval(x)
        ...
        smooth_add_Fn = Fn;
        smooth_add_Fn.f_s = @(x) Fn.f_s(x) + other_Fn.f_s(x);
        smooth_add_Fn.grad_f_s = @(x) Fn.grad_f_s(x) + other_Fn.grad_f_s(x);
        % Account for hidden oracles.
        if isfield(smooth_add_Fn, 'f_s_at_prox_f_n')
          smooth_add_Fn.f_s_at_prox_f_n = @(lam) ...
            Fn.f_s_at_prox_f_n(lam) + other_Fn.f_s_at_prox_f_n(lam);
        end
        if isfield(smooth_add_Fn, 'grad_f_s_at_prox_f_n')
          smooth_add_Fn.grad_f_s_at_prox_f_n = @(lam) ...
            Fn.grad_f_s_at_prox_f_n(lam) + other_Fn.grad_f_s_at_prox_f_n(lam);
        end
      end
    ...
    function scale(obj, alpha)
      ...
      function scaled_Fn = scaled_eval(x)
        Fn = final_eval_cpy(x);
        scaled_Fn = Fn;
        scaled_Fn.f_s = @(x) alpha * Fn.f_s(x);
        scaled_Fn.grad_f_s = @(x) alpha * Fn.grad_f_s(x);
        scaled_Fn.f_n = @(x) alpha * Fn.f_n(x);
        scaled_Fn.prox_f_n = @(x, lam) Fn.prox_f_n(x, lam * alpha);
        % Account for hidden oracles.
        if isfield(Fn, 'f_s_at_prox_f_n')
          scaled_Fn.f_s_at_prox_f_n = @(x, lam) ...
            alpha * Fn.f_s_at_prox_f_n(x, lam * alpha);
        end
        if isfield(Fn, 'f_n_at_prox_f_n')
          scaled_Fn.f_n_at_prox_f_n = @(x, lam) ...
            alpha * Fn.f_n_at_prox_f_n(x, lam * alpha);
        end
        if isfield(Fn, 'grad_f_s_at_prox_f_n')
          scaled_Fn.grad_f_s_at_prox_f_n = @(x, lam) ...
            alpha * Fn.grad_f_s_at_prox_f_n(x, lam * alpha);
        end
      end
