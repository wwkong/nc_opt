...
%
  ...
  methods
    function obj = CompModel(varargin)
      % The constructor for the CompModel class. It has three ways to  initialize: **(i)** invoking ``CompModel`` creates a
      % CompModel object with the default properties; **(ii)** invoking  ``CompModel(in_oracle)`` creates a CompModel object with
      % the oracle property set to the input oracle; and **(iii)** invoking ``CompModel(f_s, f_n, grad_f_s, prox_f_n)`` creates an 
      % Oracle object with the properties ``f_s``, ``f_n``, ``grad_f_s``, and ``prox_f_n`` filled by the corresponding input.
      if (nargin == 0) 
      elseif (nargin == 1) 
  ...
  properties (SetAccess = public)
    ...
    solver
    solver_hparams
    opt_type {mustBeMember(opt_type, {'relative', 'absolute'})} = 'absolute'
  end
  ...
  methods (Access = public, Hidden = true)
    ...
    function update_solver_inputs(obj)
      MAIN_INPUT_NAMES = {'L', 'M', 'm', 'opt_tol', 'x0', 'iter_limit', 'time_limit', 'prod_fn', 'norm_fn'};
    ...
    function update_tolerances(obj)
      ...
      if strcmp(obj.opt_type, 'relative')
        ...
        obj.internal_opt_tol = obj.internal_opt_tol * (1 + obj.norm_fn(grad_f_s_at_x0));
      end
