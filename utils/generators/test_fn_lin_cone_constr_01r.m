...
function [oracle, params] = test_fn_lin_cone_constr_01r(N, r, M, m, seed, dimM, dimN)
...
% 
  ...
  x0_base = rand(dimN, 1);
  params.x0 = x0_base / sum(x0_base) * r;
  ...
  f_s = @(x) ...
    -xi / 2 * norm_fn(D * B * x) ^ 2 + ...
     tau / 2 * norm_fn(C * x - d) ^ 2;
  f_n = @(x) 0;
  grad_f_s = @(x) ...
    -xi * B' * (D' * D) * B * x + ...
     tau * C' * (C * x -  d);
  prox_f_n = @(x, lam) sm_proj(x, r);
